#Copyright CB, 2021
#config
obvious_small_wins = True
player_symbols = ["X", "O"]
#TODO: responsive board size for infinite players
#TODO: change "space" to "cell"
#setup
import string
board = []
...
def handle_win(win_board_index):
	...
	if obvious_small_wins:
		for a in range(len(board[win_board_index])):
...
while not(end):
	...
	small_move = get_input(big_move_index, "Player " + player_symbols[current_turn] + ", which cell of board " + str(big_move) + " will you be marking? ", "That cell is taken. You must choose an empty space.")
